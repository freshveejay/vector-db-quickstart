...
class PineconeStore(BaseVectorStore):
    ...
    def _embed(self, text: str) -> List[float]:
        ...
        response = self.openai.embeddings.create(model=self.embed_model, input=text)
        return response.data[0].embedding

    def _embed_batch(self, texts: List[str]) -> List[List[float]]:
        """Generate embeddings for multiple texts"""
        response = self.openai.embeddings.create(model=self.embed_model, input=texts)
...
class VectorStore:
    ...
    PROVIDERS = {"pinecone": PineconeStore, "chroma": ChromaStore}
